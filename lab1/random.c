 ...
 *seed = rnd32 (*seed);
 u = (*seed) * RATIO;
 return ( kappa/pow(u,(1./alpha)) ); //pow(double base, double exponent)
}
...
int geometric0(double mean,long *seed)
{
 ...
 status = mean/(1.0+mean);	    /* is it a mistake? E[X] = p/(1-p) -> p = E[X]/(1+E[X])  */
...
int geometric1(double mean,long *seed)
{
 ...
 status = (mean-1)/mean;	    /* why is this different??? E[X] = 1/(1-p) -> p = (E[X]-1)/E[X]  */
